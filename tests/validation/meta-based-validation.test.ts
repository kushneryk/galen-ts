...
describe("MetaBasedValidation", () => {
  it("validates passing range", async () => {
    const r = await MetaBasedValidation.forObjects("a", "b", Range.exact(new RangeValue(10)))
      .withBothEdges(Side.LEFT)
      .validate(new Rect(20, 0, 10, 10), new Rect(10, 0, 10, 10), pv, "left");
    expect(r.error).toBeUndefined();
  });

  it("returns error for failing range", async () => {
    const r = await MetaBasedValidation.forObjects("a", "b", Range.exact(new RangeValue(5)))
      .withBothEdges(Side.LEFT)
      .validate(new Rect(20, 0, 10, 10), new Rect(10, 0, 10, 10), pv, "left");
    expect(r.error).toBeDefined();
  });

  it("computes offset using configured edges", async () => {
    const r = await MetaBasedValidation.forObjects("a", "b", Range.exact(new RangeValue(-20)))
  ...
  it("applies inverted calculation", async () => {
    const r = await MetaBasedValidation.forObjects("a", "b", Range.exact(new RangeValue(10)))
  ...
  it("generates correct meta key/value", async () => {
    const r = await MetaBasedValidation.forObjects("header", "menu", Range.exact(new RangeValue(0)))
      .withBothEdges(Side.LEFT)
  ...
  it("withBothEdges sets both first and second edge", async () => {
    const r = await MetaBasedValidation.forObjects("a", "b", Range.exact(new RangeValue(0)))
